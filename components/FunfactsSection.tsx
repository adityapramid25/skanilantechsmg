...
function AnimatedCounter({ value, suffix }: { value: number; suffix: string }) {
  ...
  return (
    <span ref={ref} className="text-3xl sm:text-4xl font-display font-bold text-slate-900 tracking-tight">
...
export function FunfactsSection() {
  return (
    <section className="py-16 bg-slate-50 border-t border-slate-200 relative overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-violet-200/50 rounded-full blur-[100px] pointer-events-none"></div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {facts.map((fact, index) => (
            <motion.div
              ...
              className="flex flex-col items-center text-center p-6 rounded-2xl bg-white border border-slate-200 hover:bg-slate-50 transition-colors shadow-sm"
            >
              <div className="w-12 h-12 rounded-full bg-violet-100 flex items-center justify-center mb-4 text-violet-600">
